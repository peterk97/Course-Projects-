 ...
 */
    ...
    public int addStock(StockItem item){
        if (item != null){
            //we would normally use get but getOrDefault will return the item if it is already exists in the map and if it is not it will use this item.
            //check if we already have quantity of this item
            StockItem inStock = list.getOrDefault(item.getName(), item);
            //if there are already stocks of this item, adjust quantity
            //if item exist we want to adjust quantity if not we just use the quantity was passed in here
            if (inStock != item){
    ...
    public int sellStock(String name, int quantity){
        StockItem inStock = list.getOrDefault(name,null); //using the same getOrDefault because we must get a result
        // because we are selling the assumption is the item has to exist so we are using default

        if ((inStock != null) && (inStock.quantityInStock() >= quantity) && (quantity > 0)){  //here we are also checking for the null regarding to inStock
        ...
        return 0;//returning null if one of the condition is not valid
    }
    ...
    public Map<String, StockItem> Items(){
        ...
        //unmodifiableMap is from the Collections class and it returns this case a Map which cannot be modified
        //so it is read only, external code cannot change it !
    }
